...
// Relaxedly load a remote Java thread's state, and determine if the thread
// could potentially be in _thread_in_Java state.
// For the purpose of epoch synchronization, we only care about never missing
// a thread that is in _thread_in_Java at the time of this state check.
// It is OK to return true for a thread that is not actually in
// _thread_in_Java state.
//
// It is possible to see the a stale value of an intermediate *_trans state,
// while the thread is currently in _thread_in_Java. This is because the
// store of _thread_in_Java is an ordinary store not followed by any fence.
// However, it is impossible to see a stale value of non-intermediate,
// not-in-Java state while the thread is in _thread_in_Java. This is because
// all transitions to _thread_in_Java must first go through a store of an
// intermediate *_trans state followed by a full fence.
// We don't need to consider _thread_new_trans or _thread_blocked_trans,
// because new and blocked states must transition to in_vm state before
// transitioning to in_Java state.
...
public:
  ...
  void do_thread(Thread* thread) {
    // We only synchronize with threads in Java state. For threads that are
    // not in Java state, they could execute the native write barrier
    // (i.e. Access::oop_store_at()). We keep the StoreLoad fence in the native
    // write barrier. Note that non-Java threads can execute the native write barrier,
    // e.g. concurrent mark worker could call ReferenceProcessor::discover_reference()
    // while scanning an InstanceRefKlass, which could store to the heap.
...
size_t G1EpochSynchronizer::async_handshake() const {
  ...
  bool valid_min_epoch = cl.valid_min_epoch();
  uintx latest_frontier = valid_min_epoch ? cl.min_epoch() : required_frontier;
  if (check_frontier_helper(latest_frontier, required_frontier)) {
    return 0;
  }
  ...
  assert(valid_min_epoch, "invariant");
  size_t count = 0;
  for (GrowableArrayIterator<JavaThread*> it = target_threads.begin();
    ...
    if (maybe_in_java_state(jt) &&
        // If there's a pending handshake operation on the target, no need to add this empty handshake.
        !jt->handshake_state()->has_operation()) {
      // Async handshake will deallocate the closure object after execution.
      Handshake::execute(new G1AsyncEpochHandshakeClosure(), jt);
      ++count;
    }
  }
  return count;
...
bool G1EpochSynchronizer::check_synchronized_inner() const {
  ...
  // If valid_min_epoch() is false, no thread is in a state that needs synced up,
  // we can safely update global frontier to required_frontier.
  uintx latest_frontier = cl.valid_min_epoch() ? cl.min_epoch() : required_frontier;
  return check_frontier_helper(latest_frontier, required_frontier);
}
